...
HashMap扩容是新建一个两倍的数组把链表复制到新数组中（重新计算下标，通过判断二进制的老容量最左位为是0还是1，是0即新数数组下标不变，是1对应新数组+16），在多个线程并发扩容时，存在线程A未完成扩容线程B又进行扩容，会在执行transfer()方法转移键值对时，造成链表成环，导致程序在执行get操作时形成死循环。
