...
- [Java](#java)
    ...
    - [集合](#集合)
        - [List](#list)
            ...
            - [ArrayList,Vector, LinkedList的存储性能和特性](#arraylistvector-linkedlist的存储性能和特性)
        - [Map](#map)
...
* 强引用（StrongReference）：就是在代码中普遍存在的，类似Object obj = new Object()这类的引用，只要强引用还存在，GC永远不会回收掉被引用的对象，当内存不足会抛出OutOfMemoryError错误。
...
防止序列化可以使用关键是transient，序列化验证使用`private static final long serialVersionUID = 1L`
...
toString():获取对象信息方法（默认返回 ToStringExample@4554617c 这种形式，其中 @ 后面的数值为散列码的无符号十六进制表示。）
...
* 局部变量表：在虚拟机栈中存储局部变量、函数调用时传递参数，很多字节码指令都是对局部变量表和操作数栈进行操作的。
...
ThreadPoolExecutor(int, int , long, TimeUnit, BlockingQueue<Runable>, ThreadFactory, RejectedExecutionHandler)
...
CAS 保证了比较和交换的原子性。但是从读取到开始比较这段期间，其他核心仍然是可以修改这个值的。如果核心将 A 修改为 B，CAS 可以判断出来。但是如果核心将 A 修改为 B 再修改回 A。那么 CAS 会认为这个值并没有被改变，从而继续操作,忽略了中间的变化过程。这是和实际情况不符的。

解决方案是加一个版本号。AtomicStampReference在cas的基础上增加了一个标记stamp，使用这个标记可以用来觉察数据是否发生变化，给数据带上了一种实效性的检验。

### JUC

#### AQS(队列同步器)

AQS是AbstractQueuedSynchronizer的缩写，用于实现基于FIFO等待队列的阻塞锁的一个同步框架，利用AQS实现的类有ReentrantLock、CountDownLatch、Semaphone。
...
* 网络层 ：为主机提供数据传输服务。而传输层协议是为主机中的进程提供数据传输服务。网络层把传输层传递下来的报文段或者用户数据报封装成分组。例如IP, ICMP。
...
能够唯一标示网络中的进程后，它们就可以利用socket进行通信了，我们经常把socket翻译为套接字，socket是在应用层和传输层之间的一个抽象层，它把TCP/IP层复杂的操作抽象为几个简单的接口供应用层调用已实现进程在网络中通信。
...
### 三次握手以及四次挥手具体过程，三次握手的原因、四次挥手原因、TIME_WAIT 的作用。
...
TCP协议包括两种窗口机制，固定窗口和滑动窗口。

固定窗口即发送方发送完数据后启动定时器，然后等待确认包，若超过一定时间没收到确认包，则重新发送该数据。

滑动窗口发送方分为四个窗口：发送且确认窗口，发送但为确认窗口，未发送但允许发送窗口，不允许发送窗口。

滑动窗口接收方分为：接受但未处理窗口，接收但未确认窗口，有空位但未接收数据窗口。
...
1. awnd：通告窗口，由接收方发给发送方，告诉发送方自己能接收多少新数据
2. cwnd：拥塞窗口，用于拥塞控制
3. ssthresh：慢启动阀值，用于确定使用慢启动还是避免拥塞
4. 发送窗口：W=min(cwnd, awnd)

TCP 主要通过四个算法来进行拥塞控制：

* 慢开始：发送网络包的大小是逐渐增长的，初始值cwnd=1，指数增长
* 拥塞避免：逐渐增长后续也会很快，设置门限到达限度后增长降低，加法增长。ssthresh=max(flight size/2, 2*SMSS)，flight size为当前发送窗口中已经发送但还没有收到ACK的包个数，即flight size<=W，SMSS为发送端最大分组大小。
* 快重传：当接收方收到的数据包是不正常的序列号，那么接收方会重复把应该收到的那一条ACK重复发送，这个时候，如果发送方收到连续3条的同一个序列号的ACK，那么就会启动快速重传机制，把这个ACK对应的发送包重新发送一次。
* 快恢复：快速恢复到正常报文速度，设置ssthresh=cwnd/2，重传丢失的包，设置cwnd=ssthresh+3，每次收到一个额外重复ACK，cwnd=cwnd+1。传输包，发现收到新的ACK则退出快恢复，进入阻塞避免。
...
解决系统间Session不共享有以下几种方案：

1. Tomcat集群Session全局复制（会影响集群性能）
2. 根据IP进行Hash映射，只访问对应机器（服务器宕机会丢失）
3. 把登陆功能拆分子系统，Session数据放在Redis中

* 单点登录即把系统用户重定向到登陆认证中心的子系统，实现多个系统只需要一个登陆。
...
1. 解析URL
   ...
   1. 浏览器通过URL获取协议类型HTTP/HTTPS，请求的资源

2. DNS分析

   1. 如果域名在浏览器缓存中直接获取IP地址，
   2. 否则进行ARP广播查询DNS服务器，通过UDP协议找到DNS服务器，获取URL域名对应的IP地址
   3. IP地址返回给浏览器，之后就可以建立HTTP连接

3. 浏览器如果使用HTTP协议则向服务器请求指定的URL资源，如果HTTPS协议，则会CA证书认证，公钥加密对称密钥，服务器私钥解密，对称密钥通信。

   1. 三次握手建立连接
   2. 四次挥手连接结束

4. 服务器把资源响应给浏览器，浏览器进行解析，渲染

* localhost不会被解析成ip，127.0.0.1等于本机通过网络访问本机
* DNS是一个分布式数据库，使用UDP或TCP进行传输，端口号53。
* 由于IP地址稀缺不可能每个人一个IP，所以通常IP地址是路由器给我们生成的IP，路由器里面的MAC地址是唯一的。
...
由于Redis所有数据都是保存在内存中的，如果不配置持久化，重启以后数据就会全部丢失。Reids提供两种持久化的方式:

* RDB：将内存中的数据库记录定时写到磁盘上RDB临时文件
* AOF：将Redis的操作日志以追加的方式写入AOF记录文件

RDB优点：恢复容易，性能最大化，启动效率更高。缺点：数据可用性不稳定，持久化过程是通过fork子进程完成，数据集较大会导致服务器停止。

AOF优点：数据持久性；缺点：文件大，运行效率慢。
...
开启慢查询（定义超过x秒就是慢查询），再利用explain关键字分析慢查询语句，分析是否索引失效。

常见优化：

1. 索引字段上进行运算会导致索引失效
2. 避免使用!=、IS NULL、IS NOT NULL、IN、NOT IN等操作符。（会无法使用索引只能搜索全表）
3. like以通配符开头(%abc....)
4. 合理使用EXISTS, NOT EXISTS
5. 少用or，用它来连接时索引会失效。
6. 减少数据量请求
...
* MySQL的InnoDB引擎在可重复读的情况下是没法保证不出现幻读的，可通过加悲观锁，即读取时即加锁。或者通过MVCC(多版本并发控制)，会给每个事务一个版本号，事务只能读取<=当前版本号的数据。（乐观锁）
...
如果直接线性存储的话，每次查找数据都要整个遍历一遍，那么复杂度就是(n)，于是可以用二叉树来存储，把复杂度降低到约log(n)，但是二叉树有个特点就是，它有可能因为插入顺序的问题，变得不平衡，最坏情况就是都在节点一边，又变成了log(n)，所有就通过改进插入和删除等操作，保证每次操作完后树都是平衡的，就有了B树……
...
1. MyISAM不支持事务，不支持外键，保存了表的行数所以使用COUNT时不会扫描全表，高性能读取
2. InnoDB支持事务处理，支持外键，高并发性能更优
...
悲观锁：使用了select…for update的方式，这样就通过数据库实现了悲观锁。其它的事务必须等本次事务提交之后才能执行。这样我们可以保证当前的数据不会被其它事务修改。InnoDB默认行锁需要where关联主键，如果没有明确主键会是表锁。

乐观锁：在update的时候需要先判断版本号，当版本号与期望一致时才进行update。update操作的时候在where条件的地方加上版本字段的相等限制，例如：
...
传统Java应用中，使用new进行bean实例化然后bean就可以使用了。

Spring生成bean：

1. 在xml中进行显示配置，Spring最原始生成Bean方式，用户只需要将类生成Bean的信息配置到XML中即可通过Spring生成Bean。（该方法逐渐被2,3代替）
2. 自动装配，只需要在类的定义上加上@Component , @Repository , @ Controller , @Service ,等注解即可让Spring自动生成Bean
3. 显示装配，通过 @Configration和@Bean注解告诉Spring帮助我们生成bean

Spring中使用Bean，即是把已经在xml文件中配置好的Bean拿来用，完成属性、方法的组装；比如@Autowired , @Resource，可以通过byTYPE（@Autowired）、byNAME（@Resource）的方式获取Bean；

例子：<https://juejin.im/post/5b1b841de51d4506d47df108#heading-6>

创建bean：创建bean实例；为bean的属性设置；调用bean的初始化方法；BeanFactory ApplicationContext

销毁bean：调用DisposableBean的destroy方法和Bean自身的destroy

作用域：

1. Singleton：单例模式，为每个IOC容器创建唯一Bean实例。
2. Prototype：原型模式，针对每个getBean请求容器单独创建一个Bean实例。
3. Request：为每个HTTP请求创建单独Bean实例。
4. Session： 为每个HTTP请求创建单独Bean实例，同时只在当前session中有效
5. GlobalSession：
...
2. 拦截器可以对Spring任何对象和资源进行使用，Filter则只能在Servlet前后使用

## Web应用部署过程

当一个Web应用部署到容器(tomcat)内，会执行以下步骤：
1. 部署描述文件中(tomcat的web.xml)由<listener>元素标记的事件监听器会被创建和初始化
2. 对于所有事件监听器，如果实现了ServletContextListener接口，将会执行其实现的contextInitialized()方法
3. 部署描述文件中由<filter>元素标记的过滤器会被创建和初始化，并调用其init()方法
4. 部署描述文件中由<servlet>元素标记的servlet会根据<load-on-startup>的权值按顺序创建和初始化，并调用其init()方法

总体流程即：初始化listener->初始化filter->初始化servlet
...
Spring MVC启动过程：首先tomcat启动会去读取web.xml配置文件，关组件启动顺序为: 解析<context-param> => 解析<listener> => 解析<filter> => 解析<servlet>

四大组件:DispatcherServlet、HandlerMapping、HandlerAdapter以及ViewResolver

运行过程如下：
1. Web浏览器发送HTTP请求到服务端，被Controller(DispatcherServlet)获取并进行处理（例如参数解析、请求转发） 
...
使用 Spring Boot 自动配置后，只需要添加 MVC 组件即可自动配置所需要的 Bean。所有自动配置的实现都在 spring-boot-autoconfigure 依赖中。
...
### IO多路复用select, poll, epoll
 ...
 select 更加适用于实时性要求比较高的场景,poll 没有最大描述符数量的限制，如果平台支持并且对实时性要求不高，应该使用 poll 而不是 select。
...
三套生命周期：clean（清理项目），default（构建项目），site（建立发布项目站点）