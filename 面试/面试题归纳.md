...
HashMap扩容是新建一个两倍的数组把链表复制到两个数组中（通过hash值第X为是0还是1）
ConcurrentHashMap在1.7中由于桶是无法扩容的，所以扩容的其实是一个桶中的数组。jdk8中，采用多线程扩容。整个扩容过程，通过CAS设置sizeCtl，transferIndex等变量协调多个线程进行并发扩容。
...
* 1.8中放弃了Segment臃肿的设计，取而代之的是采用Node + CAS + Synchronized。
* put：当执行put方法插入数据时，根据key的hash值，在Node数组中找到相应的位置,如果相应位置的Node还未初始化，则通过CAS插入相应的数据；如果相应位置的Node不为空，且当前该节点不处于移动状态，则对该节点加synchronized锁，如果该节点的hash不小于0，则遍历链表更新节点或插入新节点；
...
AQS是AbstractQueuedSynchronizer的缩写，用于实现基于FIFO等待队列的阻塞锁和相关的同步器的一个同步框架，利用AQS实现的同步工具类有CountDownLatch、ReentrantLock、Semaphone。

CountDownLatch：用来控制一个线程等待多个线程，维护了一个计数器cnt，每次调用countDown()方法会让计数器值减1。
...
1. 先使用DNS协议获取URL域名对应的IP地址，之后就可以建立HTTP连接
2. 浏览器使用HTTP协议向服务器请求指定的URL资源
3. 由于HTTP是基于TCP连接之上的，所以会涉及TCP三次握手四次挥手。
4. 服务器把资源响应给浏览器，浏览器进行渲染
...
概念：https://www.cnblogs.com/bossen/p/5824067.html

创建bean：创建bean实例；为bean的属性设置；调用bean的初始化方法；

销毁bean：调用DisposableBean的destroy方法和Bean自身的destroy

作用域：1.Singleton为每个IOC容器创建唯一Bean实例。2.Prototype针对每个getBean请求容器单独创建一个Bean实例。如果是web容器还有：3.Request为每个HTTP请求创建单独Bean实例。4.Session 5.GlobalSession
