...
区别：
* 抽象类中可以包含非抽象的普通方法，接口中的所有方法必须都是抽象方法。
...
#### HashMap链表环，定位逻辑，线程安全方法

定位：1.计算key的hash值 2.根据hash确定在table数组中的位置 3.判断数组的Node对象中key是否等同与传入的key 4.若不是，则一次扫描next节点的key，直到找到为止。O(1)的时间复杂度。

HashMap扩容是新建一个两倍的数组把链表复制到两个数组中（通过hash值第X为是0还是1），在多个线程并发扩容时，存在线程A未完成扩容线程B又进行扩容，会在执行transfer()方法转移键值对时，造成链表成环，导致程序在执行get操作时形成死循环。
...
是所有类的父类，三个常用方法：toString()、equals()、hashCode()

toString():获取对象信息方法（地址）
