...
从前往后依次比较相邻的两个数大小，如果前者比后者大就交换位置。这样一次遍历完最大元素就在末尾，重复直到整个数列有序。
...
从数列中挑出一个基准值，把基准值小的放前面，比基准值大的放后面，

平均时间复杂度：O(N*lgN)，最坏情况O(N^2)
...
把n个待排序元素看成一个有序表和无序表，有序表只包含1个元素，无序表包含n-1个元素，每次从无序表中取出第一个元素，放入到有序表中适当位置。
...
选取步长gap，把数列分成gap个组，每组数组进行排序，gap缩短二分之一，直到小于零

时间复杂度：O(N^(3/2)),如果gap为1则退化成了直接插入排序
