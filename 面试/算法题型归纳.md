...
需要使用一个pre连接head，可以解决每次两两交换完的链表的连接问题，pre->l1->l2->next ==>  pre->l2->l1->next==>l2->l1/pre->
...
2. 在左右子树中查找是否存在 p 或者 q，如果 p 和 q 分别在两个子树中，那么就说明根节点就是最低公共祖先。
