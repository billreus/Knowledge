...
在String需要按数组取出的时候多用`xxx.charAt(i)`来实现而不是转化成char
...
由于需要所有种大小写的可能，所以使用dfs搜索实现的溯源算法
...
class Solution {
    ...
    void dfs(char []chars, int index, List<String> list)
    {
        ...
        char c = chars[index];
        dfs(chars, index+1, list);//1
        if(chars[index] >= 'a')
        {
            chars[index] = (char)(chars[index] - 32);
            dfs(chars, index+1, list);//2
        }
        else if(chars[index] >= 'A')
        {
          chars[index] =  (char)(chars[index] + 32);
            dfs(chars, index+1, list);//3
        }
